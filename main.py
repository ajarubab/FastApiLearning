...
products = [
    ...
    Product(
        id=102, name="pencil", price=5.5, quantity=200, description="Natraj pencil"
    ),
    Product(id=103, name="Eraser", price=7.5, quantity=200, description="20gms eraser"),
    Product(
        id=104, name="Sharpner", price=8, quantity=200, description="quicky sharpner"
    ),
]
...
def add_more_products(nw_pdt: List[Product], db: Session = Depends(get_db)):
    ...
    db.add_all(pdts)
    db.commit()
    return {"message": f"Successfully added {len(nw_pdt)} product(s)", "added": nw_pdt}
...
def update_product(id: int, pdt: Product, db: Session = Depends(get_db)):
    ...
    """
    for key, value in pdt.model_dump().items():
...
@app.patch("/update/{id}")
def update_products(id: int, pdt: ProductUpdate, db: Session = Depends(get_db)):
    db_pdt = db.query(dbModels.Product).filter(dbModels.Product.id == id).first()
...
def delete_product(id: int, db: Session = Depends(get_db)):
    ...
    if db_pdt:
        db.delete(db_pdt)
        db.commit()
        return {"Mesage": "Product deleted successfully", "Deleted": db_pdt}
    return {"error": "Product not found"}
